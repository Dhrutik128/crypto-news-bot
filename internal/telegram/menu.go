...
func InitHandler(bot *tb.Bot, db *storage.DB, newsfeed *news.Analyzer) {
	bot.Handle("/start", func(m *tb.Message) {
		...
		_, err := bot.Send(m.Sender, markdownEscape("HI! I am a *Crypto News Stream Bot*. \n\n"+
			"I will fetch the latest news from RSS feeds and categorize them grouped by coin.\n"+
	...
	bot.Handle(&btnGit, func(m *tb.Message) {
		config.IgnoreErrorMultiReturn(bot.Send(m.Sender, "https://github.com/gohumble/crypto-news-bot"))
	})

	bot.Handle(&btnHelp, func(m *tb.Message) {
		config.IgnoreErrorMultiReturn(bot.Send(m.Sender, "Usage:\n\n"+
	...
	bot.Handle(&btnSentiments, func(m *tb.Message) {
		config.IgnoreErrorMultiReturn(bot.Send(m.Sender, newsfeed.GetSentimentTable(), &tb.SendOptions{
